...
def get_configs(args):
    ...
    multiclass_configs = {
        ...
        'ViT-B/16': {
            ...
            'PLTNetMini': {'lamb_preserve': 0.25, 'lamb_forget': 1.1, 'lora_r': 5, 'lamb_weight': 1.},
        }
    }
...
def get_model(arch="RN50", device='cpu', load_path="", lr=5e-5):
    ...
    model = create_bioclip_model(arch=arch, device=device, load_path=load_path)
...
def evaluate_clip_zs(model, loader, clip_weights, device=None, out_conf=False, output_probs=False):
    ...
    print(f"Debug evaluation shapes: features={features.shape}, clip_weights={clip_weights_tensor.shape}")
    
    if clip_weights_tensor.shape[0] != features.shape[1]:
        # Need to transpose weights for proper matrix multiplication
        clip_weights_tensor = clip_weights_tensor.T
        print(f"Transposed clip_weights to: {clip_weights_tensor.shape}")
